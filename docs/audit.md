...
- [x] **Transactional crawl write** — per-URL `IServiceScope` + `IUnitOfWork.BeginTransactionAsync` wraps `SaveUrlAsync` → `SaveRedirectChainAsync` → `PersistLinkRelationshipsAsync` → `CrawlQueueRepository.UpdateAsync(queueItem)` in `src/ShoutingIguana.Core/Services/CrawlEngine.cs:522-552`. Commit on success, rollback + rethrow on failure. New-URL enqueue (`ExtractAndEnqueueNewLinksAsync`) runs post-commit in its own scope (`CrawlEngine.cs:554-557`) so "discover a new URL" is not atomic with "finish this one" — keeps SQLite writer contention low.
...
- [x] **`IHttpClientFactory` adoption** — `RobotsService` and `SitemapService` use the injected `IHttpClientFactory`. `CrawlEngine.FetchStaticResourceAsync` uses the shared `"crawl-static"` named client for the no-proxy path. The proxy-active path now caches one `HttpClient` per distinct proxy config (`ConcurrentDictionary` keyed on `Type|Server|Port|Username|EncryptedPassword|BypassList`), built on first use via `GetStaticResourceHttpClient`/`BuildProxiedStaticClient` and reused for the remainder of the crawl so the socket pool is retained instead of rebuilt per URL. The cache is disposed + cleared in `StartCrawlAsync` and in `StopCrawlAsync`'s finally block. Per-proxy-named-client via the factory would be equivalent but wasn't chosen because the proxy config is discovered per-project at runtime — a dictionary keyed on that config is the direct equivalent.
...
- [x] **Stale `net9.0` artefact cleanup** — `dotnet clean ShoutingIguana.sln` only cleans outputs for the current `TargetFramework`, so it left the prior-TFM `net9.0` / `net9.0-windows` directories orphaned under `bin/` and `obj/` across every project. Those were removed manually. Remaining `net9.0` hits under `bin/**/net10.0*/runtimes/browser-wasm/nativeassets/net9.0/` are legitimate — they are the sqlite native-asset layout nested inside the net10 build output, not stale artefacts.
