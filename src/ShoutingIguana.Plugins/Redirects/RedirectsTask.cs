...
public class RedirectsTask(ILogger logger, IRepositoryAccessor repositoryAccessor) : UrlTaskBase
{
    ...
    private async Task AnalyzeHttpRedirectAsync(UrlContext ctx, string? targetUrl)
    {
        ...
        await ReportRowAsync(ctx, redirectRow);
    ...
    private async Task ReportRedirectChainAsync(UrlContext ctx, List<RedirectHop> chain, bool isLoop)
    {
        ...
        var severity = isLoop
            ? Severity.Error
            : hopCount >= MAX_REDIRECT_CHAIN_LENGTH ? Severity.Error
            : hopCount >= WARNING_REDIRECT_CHAIN_LENGTH ? Severity.Warning
            : Severity.Info;
    ...
    private async Task ValidateRedirectTargetAsync(UrlContext ctx, string? targetUrl, CancellationToken ct)
    {
        ...
        if (status >= 400)
        {
            var severity = status >= 500 ? Severity.Error : Severity.Warning;
    ...
    /// </summary>
    private Task ReportRowAsync(UrlContext ctx, ReportRow row, string? explanation = null)
    {
        var message = string.IsNullOrWhiteSpace(explanation)
            ? row.Get("Issue")?.ToString()
            : explanation;

        if (!string.IsNullOrWhiteSpace(message))
        {
            row.SetExplanation(message);
        }
