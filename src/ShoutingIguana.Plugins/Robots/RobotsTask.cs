...
public class RobotsTask(ILogger logger) : UrlTaskBase
{
        ...
        try
        {
            ...
            if (!status.Exists && !status.ReportedMissing)
            {
                var row = ReportRow.Create()
                    ...
                    .SetExplanation( "Search engines have no robots.txt guidance, so add one to control what should or shouldn't be crawled.")
    ...
    private async Task AnalyzeXRobotsTagAsync(UrlContext ctx, bool isIndexable)
    {
        ...
        var row = ReportRow.Create()
            ...
            .SetExplanation( "An X-Robots-Tag header is being sent, which can override HTML meta robots directives; make sure it matches your desired indexability.")
    ...
    private async Task CheckRobotsConflictsAsync(UrlContext ctx)
    {
        ...
        var row = ReportRow.Create()
            ...
            .SetExplanation( "Meta robots and X-Robots-Tag directives disagree, which can cause inconsistent behavior in search engines.")
    ...
    private async Task CheckImportantPageIndexabilityAsync(UrlContext ctx, bool isIndexable)
    {
        ...
        if (!isIndexable && ctx.Metadata.Depth <= 2)
        {
            ...
            var row = ReportRow.Create()
                ...
                .SetExplanation( importantPageDescription)
    ...
    private async Task CheckAdvancedRobotsDirectivesAsync(UrlContext ctx)
    {
        ...
        if (tag.Contains("noarchive"))
        {
                var row1 = ReportRow.Create()
                    ...
                    .SetExplanation( "Search engines are instructed not to keep a cached copy of this page.")
        ...
        if (tag.Contains("nosnippet"))
        {
            var row = ReportRow.Create()
                ...
                .SetExplanation( "No snippet means search results cannot show descriptive text for this page, which may reduce clicks.")
                .Set("RobotsMeta", "(header only)")
        ...
        if (tag.Contains("noimageindex"))
        {
            var row = ReportRow.Create()
                ...
                .SetExplanation( "Images on this page are excluded from Google Image Search.")
                .Set("RobotsMeta", "(header only)")
        ...
        if (tag.Contains("unavailable_after"))
        {
            var row = ReportRow.Create()
                ...
                .SetExplanation( "This header schedules the page to be removed from search results after the specified time.")
                .Set("RobotsMeta", "(header only)")
        ...
        if (tag.Contains("indexifembedded"))
        {
            var row = ReportRow.Create()
                ...
                .SetExplanation( "This page will only be indexed when embedded via an iframe, not when viewed on its own.")
                .Set("RobotsMeta", "(header only)")
        ...
        if (maxSnippetMatch.Success)
        {
            ...
            var row2 = ReportRow.Create()
                ...
                .SetExplanation( snippetDescription)
                .Set("RobotsMeta", "(header only)")
        ...
        if (maxImageMatch.Success)
        {
            ...
            var row3 = ReportRow.Create()
                ...
                .SetExplanation( imageDescription)
                .Set("RobotsMeta", "(header only)")
        ...
        if (maxVideoMatch.Success)
        {
            ...
            var row4 = ReportRow.Create()
                ...
                .SetExplanation( videoDescription)
                .Set("RobotsMeta", "(header only)")
        ...
        if (tag.Contains("googlebot-news"))
        {
            var row5 = ReportRow.Create()
                ...
                .SetExplanation( "This directive applies only when submitting to Google News; keep it only if you intend to target Google News.")
                .Set("RobotsMeta", "(header only)")
